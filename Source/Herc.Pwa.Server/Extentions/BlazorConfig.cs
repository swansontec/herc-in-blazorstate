...
namespace Microsoft.AspNetCore.Components.Server
{
  internal class BlazorConfig
  {
    ...
    private BlazorConfig(string assemblyPath)
    {
      ...
      string configFilePath = Path.ChangeExtension(assemblyPath, ".blazor.config");
      ...
      string sourceMsBuildDir = Path.GetDirectoryName(SourceMSBuildPath);
      SourceOutputAssemblyPath = Path.Combine(sourceMsBuildDir, configLines[1]);

      string webRootPath = Path.Combine(sourceMsBuildDir, "wwwroot");
